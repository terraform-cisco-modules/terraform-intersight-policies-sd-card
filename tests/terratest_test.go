...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-sd-card-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} SD Card Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/sdcard/Policies/%s", moid), expectedJSONTemplate, vars)
}
